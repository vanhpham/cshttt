...
% Draw paths to receiver (placed 5.5cm to the right of tx antennas, centered at y=0)
\node [ant, shape border rotate=270, right=5.5cm of tx1, yshift=-1.3cm] (rx1) {};
\node [right=0.2cm of rx1, font=\footnotesize\bfseries] {Receiver ($Rx_i$)};

% Signal arrows representing channel paths h_i1 and h_i2
\draw [arrow, dashed, green!60!black] (tx1.east) -- node[above, sloped, font=\tiny\bfseries]{$h_{i1}$ (Path 1)} (rx1.west);
\draw [arrow, dashed, blue!60!black] (tx2.east) -- node[below, sloped, font=\tiny\bfseries]{$h_{i2}$ (Path 2)} (rx1.west);
