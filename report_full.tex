...
OFDM improves system performance in the multipath Rayleigh channel. The IFFT/FFT structure divides the frequency-selective channel into multiple narrowband subchannels, while the cyclic prefix helps preserve the circular convolution property of the channel. In this simulation, each OFDM block is processed independently. Therefore, the cyclic prefix primarily ensures that frequency-domain subcarriers are perfectly orthogonal for single-tap equalization, rather than mitigating continuous inter-block ISI. In a continuous transmission system, the cyclic prefix would also prevent inter-symbol interference (ISI) between adjacent OFDM symbols. A continuous frame-based simulation could be considered in future work to explicitly observe and model continuous block-to-block ISI.
