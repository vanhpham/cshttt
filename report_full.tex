...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textit{Serial to Parallel (S/P) Converter:} Splits the serial bit stream into parallel blocks to feed the mapper.
        \item \textit{16-QAM Mapper:} Groups the bits into 4-bit symbols and maps them onto the complex 16-QAM constellation.
        \item \textit{Alamouti STBC Encoder:} Encodes the mapped symbols in pairs ($s_1, s_2$) across two spatial paths and two time slots.
    ...
    \item \textbf{Receiver:}
    \begin{enumerate}
        \item \textit{OFDM Demodulator (Remove CP + FFT):} Discards the CP and applies an $N_{fft}$-point Fast Fourier Transform (FFT) to return the signals to the frequency domain.
        ...
        \item \textit{Parallel to Serial (P/S) Converter:} Restores the single serial bit stream for final performance evaluation.
    \end{enumerate}
\end{itemize}
